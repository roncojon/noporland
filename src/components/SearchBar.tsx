...
const SearchBar: React.FC<SearchBarProps> = ({ tags, onSearch }) => {
  ...
  return (
    <div className="search-bar-container w-full mt-4"> {/*  mb-12 */}
      {/* Search Input */}
      <div className="relative">
        ...
        {(searchTerm || selectedTags.length>0) && (
          <button
            className="absolute right-[86px] top-[2px] text-gray-400 hover:text-gray-600 p-2 pt-1 "
        ...
        {/* Tags Dropdown */}
        {isDropdownOpen && (
          <div className="absolute w-full mt-1 bg-white shadow-lg border rounded-md p-2 z-20">
            {tags.map(tag => (
              <div
                key={tag.id}
                className={`badge cursor-pointer p-3 m-1 ${
      ...
      <div className="flex flex-wrap my-2">
        {selectedTags.map(tag => (
          <div
            key={tag.id}
            className="badge badge-primary flex items-center p-3 m-1 text-white"
            tabIndex={0} // Allow focusing the tag chips for keyboard accessibility
            onKeyDown={(e) => handleKeyDown(e)} // Trigger search on Enter key for tags
          >
            {tag.name}
